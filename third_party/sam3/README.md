...
# cc_torch builds a CUDA extension, so make sure `nvcc` is on PATH and `CUDA_HOME` points to your CUDA toolkit
# (for example, on module-based clusters: `module load cuda/12.8`)
# cc_torch also imports torch during its build, so disable build isolation to reuse the torch in this env
