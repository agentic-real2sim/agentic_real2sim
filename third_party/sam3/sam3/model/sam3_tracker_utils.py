...
def fill_holes_in_mask_scores(
    ...
    Make sure the CUDA toolkit is available first so that `nvcc` is on PATH and `CUDA_HOME`
    is set (for example, by loading your CUDA module on cluster environments):
    ```
    pip uninstall -y cc_torch && TORCH_CUDA_ARCH_LIST="8.0 9.0" pip install --no-build-isolation git+https://github.com/ronghanghu/cc_torch.git
    ```
