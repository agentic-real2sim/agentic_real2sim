...
class FoundationPose:
  ...
  def reset_object(self, model_pts, model_normals, symmetry_tfs=None, mesh=None):
    ...
    if self.mesh is not None:
      export_root = os.path.join(self.debug_dir, "_mesh_exports")
      try:
        os.makedirs(export_root, exist_ok=True)
      except PermissionError:
        export_root = tempfile.gettempdir()
      mesh_export_dir = os.path.join(export_root, str(uuid.uuid4()))
      os.makedirs(mesh_export_dir, exist_ok=True)
      self.mesh_path = os.path.join(mesh_export_dir, 'model.obj')
