"""Run inference using a trained Warp model checkpoint."""
import dataclasses
import glob
...
if __name__ == "__main__":
    ...
    best_model_paths = sorted(
        glob.glob(f"{base_dir}/train/best_*.pth"),
        key=lambda p: int(Path(p).stem.split("_")[1]),
    )
    assert len(best_model_paths) > 0, f"No checkpoint found in {base_dir}/train/"
    trainer.test(best_model_paths[-1])
