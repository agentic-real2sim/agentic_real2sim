...
class SVOReader:
    def __init__(self, filepath, serial_number):
        ...
        # Disable neural depth — we only need rectified RGB + K, and the
        # default DEPTH_MODE.NEURAL tries to load model weights that aren't
        # present under ZED SDK's `skip_python` silent install, hanging open()
        # for ~2.5min before raising CORRUPTED SDK INSTALLATION.
        init_parameters.depth_mode = sl.DEPTH_MODE.NONE
        init_parameters.camera_image_flip = sl.FLIP_MODE.OFF
        init_parameters.set_from_svo_file(filepath)
        # Point SDK at a local SN<serial>.conf cache so it doesn't try to
        # download calibration from calib.stereolabs.com (fails on hosts
        # without internet, or for retired serials). The cache dir defaults
        # to ~/Research/DROID/data/zed_calib/; override with ZED_CALIB_DIR.
        # Trailing "/" is required — SDK naively concatenates path +
        # "SN<serial>.conf" without inserting a separator.
        import os as _os
        _calib_dir = _os.environ.get(
            "ZED_CALIB_DIR",
            _os.path.expanduser("~/Research/DROID/data/zed_calib"),
        )
        if _os.path.isdir(_calib_dir):
            init_parameters.optional_settings_path = _os.path.join(_calib_dir, "")
    ...
    def set_reading_parameters(
        ...
        self.rectified_images = rectified_images
    ...
    def get_image_timestamp_ns(self):
        """Image timestamp of the frame the last read_camera() grabbed, in ns.

        read_camera(return_timestamp=True) rounds to milliseconds, which is
        coarser than the SVO records and coarser than the robot stream a
        self-collected episode has to be paired against.
        """
        return self._cam.get_timestamp(sl.TIME_REFERENCE.IMAGE).get_nanoseconds()
    ...
    def read_camera(self, ignore_data=False, correct_timestamp=None, return_timestamp=False):
        ...
        if self.image:
            if self.concatenate_images:
                ...
                left_view = sl.VIEW.LEFT if self.rectified_images else sl.VIEW.LEFT_UNRECTIFIED
                right_view = sl.VIEW.RIGHT if self.rectified_images else sl.VIEW.RIGHT_UNRECTIFIED
                self._cam.retrieve_image(self._left_img, left_view, resolution=self.zed_resolution)
                self._cam.retrieve_image(self._right_img, right_view, resolution=self.zed_resolution)
                data_dict["image"] = {
