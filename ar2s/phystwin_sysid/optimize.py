...
class OptimizerCMA:
    def __init__(
        ...
        if not os.path.exists(f"{self.base_dir}/optimizeCMA"):
            # Create directory if it doesn't exist
            os.makedirs(f"{self.base_dir}/optimizeCMA")
    ...
    def optimize(self, max_iter=100):
        ...
        init_global_spring_Y = self.normalize(
            self.cfg.init_spring_Y, self.cfg.spring_Y_min, self.cfg.spring_Y_max
        )
        init_object_radius = self.normalize(self.cfg.object_radius, 0.01, 0.05)
        init_object_max_neighbours = self.normalize(self.cfg.object_max_neighbours, 10, 50)
        init_controller_radius = self.normalize(self.cfg.controller_radius, 0.01, 0.08)
        init_controller_max_neighbours = self.normalize(
            self.cfg.controller_max_neighbours, 10, 80
        )
        init_collide_elas = self.cfg.collide_elas
        init_collide_fric = self.normalize(self.cfg.collide_fric, 0, 2)
        init_collide_object_elas = self.cfg.collide_object_elas
        init_collide_object_fric = self.normalize(self.cfg.collide_object_fric, 0, 2)
        init_collision_dist = self.normalize(self.cfg.collision_dist, 0.01, 0.05)
        init_drag_damping = self.normalize(self.cfg.drag_damping, 0, 20)
        init_dashpot_damping = self.normalize(self.cfg.dashpot_damping, 0, 200)
        ...
        self.error_func(
            x_init, visualize=True, video_path=f"{self.base_dir}/optimizeCMA/init.mp4"
        )
        ...
        final_global_spring_Y = self.denormalize(
            optimal_x[0], self.cfg.spring_Y_min, self.cfg.spring_Y_max
        )
        ...
        self.error_func(
            optimal_x,
            visualize=True,
            video_path=f"{self.base_dir}/optimizeCMA/optimal.mp4",
        )
        ...
        with open(f"{self.base_dir}/optimal_params.pkl", "wb") as f:
    ...
    def error_func(self, parameters, visualize=False, video_path=None):
        global_spring_Y = self.denormalize(
            parameters[0], self.cfg.spring_Y_min, self.cfg.spring_Y_max
        )
