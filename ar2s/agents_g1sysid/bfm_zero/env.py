...
_BFM_ROOT = os.path.dirname(os.path.abspath(__file__))
...
class MuJoCoBFMZeroEnv:
    ...
    def __init__(self, robot_xml,
        ...
        if enable_video:
            try:
                ...
                # Free camera that smoothly follows the robot's position
                # without rotating — eliminates the "swinging" effect caused
                # by mjCAMERA_TRACKING panning as the robot translates.
                self.camera = mujoco.MjvCamera()
                self.camera.type      = mujoco.mjtCamera.mjCAMERA_FREE
                self.camera.distance  = 3.0
                self.camera.elevation = -15.0
                self.camera.azimuth   = 180.0
                # lookat is updated every step in _update_camera_lookat()
    ...
    def step(self, action):
        ...
        if self.enable_video and self.renderer is not None and self.camera is not None:
            # Keep free camera centred on the pelvis without rotating
            if self.track_body_id is not None:
                self.camera.lookat[:] = self.mjd.xpos[self.track_body_id]
    ...
    def get_image(self):
        ...
        if self.renderer is None:
            return None
        if self.camera is not None and self.track_body_id is not None:
            self.camera.lookat[:] = self.mjd.xpos[self.track_body_id]
        self.renderer.update_scene(self.mjd, camera=self.camera)
...
def apply_random_physics_perturbation(
    env: "MuJoCoBFMZeroEnv",
    seed: int | None = None,
    *,
    mass_log10_range: tuple[float, float] = (-0.6, 1.0),
    friction_scale_range: tuple[float, float] = (15.0, 120.0),
    pd_gain_scale_range: tuple[float, float] = (0.03, 0.35),
) -> None:
    """
    Randomly perturb MuJoCo bodies, joint friction, and PD gains so control / motion often fails.

    Intended for debugging or robustness experiments — not for training.

    Call after ``MuJoCoBFMZeroEnv`` is constructed, then ``env.reset()`` before rolling out.

    - Scales each body mass (skips world) by ``10**U(mass_log10_range)``
    - Multiplies each DOF ``frictionloss`` by ``U(friction_scale_range)``
    - Scales ``env.kp_gains`` and ``env.kd_gains`` by one ``U(pd_gain_scale_range)`` factor
    """
    rng = np.random.default_rng(seed)
    m = env.mjm
    for i in range(1, m.nbody):
        factor = 10 ** rng.uniform(*mass_log10_range)
        m.body_mass[i] *= float(factor)
    for i in range(m.nv):
        m.dof_frictionloss[i] *= float(rng.uniform(*friction_scale_range))
    scale = float(rng.uniform(*pd_gain_scale_range))
    env.kp_gains = (env.kp_gains * scale).astype(np.float32)
    env.kd_gains = (env.kd_gains * scale).astype(np.float32)
    mujoco.mj_forward(m, env.mjd)
